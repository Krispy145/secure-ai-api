...
SAMPLE_DOCUMENTS: List[Dict[str, Any]] = [
    {
        ...
        "text": (
            ...
            "on the victim's machine. Common phishing indicators include: suspicious sender "
            "addresses, urgent language, requests for sensitive information, and suspicious "
            "links or attachments."
        ),
        "metadata": {
            "source": "cybersecurity_guide.pdf",
            "topic": "phishing",
            "type": "educational",
        },
    },
    {
        "id": "doc_phishing_2",
        "text": (
            "Phishing detection techniques include: checking URL authenticity, verifying "
            "sender identity, examining email headers, looking for spelling and grammar "
            "errors, and being cautious of unsolicited requests. Machine learning models "
            "can analyze URL features, email content, "
            "and behavioral patterns to automatically detect phishing attempts."
        ),
        "metadata": {
            "source": "phishing_detection_handbook.pdf",
            "topic": "phishing",
            "type": "technical",
        },
    },
    {
        "id": "doc_security_1",
        "text": (
            "API security best practices include: implementing authentication and "
            "authorization, using HTTPS for all communications, validating and sanitizing "
            "all inputs, "
            "implementing rate limiting to prevent abuse, logging and monitoring API "
            "access, and keeping "
            "dependencies up to date."
        ),
        "metadata": {
            "source": "api_security_guide.pdf",
            "topic": "api_security",
            "type": "best_practices",
        },
    },
    {
        ...
        "text": (
            ...
            "for generating accurate, contextual responses. RAG systems typically use vector "
            "databases "
...
def keyword_retrieve(
    query: str, documents: List[Dict[str, Any]], n_results: int = 3
) -> List[Dict[str, Any]]:
    """Score in-memory docs by overlapping query terms (MVP retrieval fallback)."""
...
def seed_sample_data_if_empty(service: "RAGService") -> bool:
    """Ingest SAMPLE_DOCUMENTS when the vector collection is empty."""
    try:
        if not service.is_initialized or not service.collection:
            return False
        if service.collection.count() > 0:
            return True
        logger.info("Initializing RAG service with sample data...")
        return service.initialize_with_sample_data()
...
class RAGService:
    ...
    def __init__(
        ...
        self.persist_directory = (
            persist_directory or Path(__file__).parent.parent / "data" / "chroma"
        )
    ...
    def query(
        self, query_text: str, n_results: int = 3, max_tokens: int = 500, temperature: float = 0.7
    ) -> Dict[str, Any]:
        """
    ...
    def _retrieve_documents(self, query: str, n_results: int = 3) -> List[Dict[str, Any]]:
        ...
        if not self.collection or not self.embedding_model:
            return keyword_retrieve(query, SAMPLE_DOCUMENTS, n_results)
        ...
        try:
            ...
            return documents or keyword_retrieve(query, SAMPLE_DOCUMENTS, n_results)
    ...
    def _fallback_response(self, query: str) -> Dict[str, Any]:
        ...
        if "phishing" in query_lower or "security" in query_lower:
            ...
            response = (
                "This API provides endpoints for phishing detection and RAG-based "
                "question answering. The phishing endpoint accepts URLs and returns "
                "classification results. The RAG endpoint processes natural language "
                "queries and returns contextual responses."
            )
            confidence = 0.80
        else:
            response = (
...
def get_rag_service() -> RAGService:
    ...
    if _rag_service is None:
        ...
        seed_sample_data_if_empty(_rag_service)
    return _rag_service
