...
class QueryRequest(BaseModel):
    ...
    max_tokens: Optional[int] = Field(
        default=500, ge=1, le=2000, description="Maximum tokens in response"
    )
    temperature: Optional[float] = Field(
        default=0.7, ge=0.0, le=2.0, description="Sampling temperature"
    )
    n_results: Optional[int] = Field(
        default=3, ge=1, le=10, description="Number of relevant documents to retrieve"
    )
...
async def rag_query(request: Request, response: Response, payload: QueryRequest):
    """
